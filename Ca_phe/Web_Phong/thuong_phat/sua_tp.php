...
try {
    ...
    if (!empty($_POST['submit'])) {
        ...
        $sql = "UPDATE thuong_phat SET idnhan_vien='$idnhan_vien', phanloai='$phanloai',  thoigian=CURRENT_TIMESTAMP, sotien='$sotien', noidung='$noidung' WHERE idtinh_huong='$idtinh_huong'";
        $stmt = $conn->prepare($sql);
        $query = $stmt->execute();
